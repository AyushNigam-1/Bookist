...
from gpt_scrape import GPTChat
load_dotenv()
gpt_scraper = GPTChat()
...
class BookistProcessor:
    ...
    def extract_actionable_steps(self, text_chunk):
        ...
        #         response = gpt_scraper.chat_with_gpt(prompt)

        response = gpt_scraper.chat_with_gpt(prompt)
        new_data = extract_json_from_markdown(response)
    ...
    def categorize_steps(self): 
        ...
        new_categories = extract_json_from_markdown(response)
    ...
    def order_hierarchy(self):
        ...
        ordered_topics = json.loads(response)
